# Path Alias

# Workspace
hash -d ws="$HOME/Workspace"
hash -d dot="$HOME/Workspace/dotfiles"
hash -d jg="$HOME/Workspace/js-geek"
hash -d vg="$HOME/Workspace/vim-geek"
hash -d sl="$HOME/Workspace/sl"
hash -d q="$HOME/Workspace/OpenQuant/nine-zen"
# hash -d ad="$HOME/Workspace/VimProjects/Amazon/allbe-diamond"

# brew 
hash -d br="/opt/homebrew/Cellar"
hash -d brc="/opt/homebrew/Caskroom/"

# node_modules
hash -d npm="$NVM_BIN/../lib/node_modules"
hash -d pnpm="$PNPM_HOME/global/5/node_modules"

# Home
hash -d doc="$HOME/Documents"
hash -d doc="$HOME/Library"

# Config
hash -d zconf="$DOTFILES/zsh/custom"

# Vim 
hash -d vplug="$HOME/.cache/vimfiles/repos/github.com"
hash -d vcoc="$HOME/.config/coc"
hash -d vsp="$HOME/.SpaceVim"
hash -d vconf="$HOME/.SpaceVim.d"

# zinit
hash -d zplug="$HOME/.zinit/plugins"
