...
# [A]pt [I]nstall
function zai() {
  local packages=$(apt list | eval $APT_FORMAT | fzf-tmux -m --query="$1" --preview 'apt show {}')
...
# [A]pt [R]emove
function zar() {
  local packages=$(apt list --installed | eval $APT_FORMAT | fzf-tmux -m --query="$1" --preview 'apt show {}')
  if [[ $packages ]]; then
    local packagesStr=$(echo $packages | eval $AWK_JOIN)
