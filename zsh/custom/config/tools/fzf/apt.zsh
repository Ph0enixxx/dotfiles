...
# [A]pt [I]nstall
function zai() {
  local packages=$(apt list | eval $APT_FORMAT | fzf-tmux -m --query="$1" --preview 'apt list -a {} && apt show -a {}')
...
# [A]pt [R]emove
function zar() {
  local packages=$(apt list --installed | eval $APT_FORMAT | fzf-tmux -m --query="$1" --preview 'apt list -a {} && apt show -a {}')
  if [[ $packages ]]; then
    local packagesStr=$(echo $packages | eval $AWK_JOIN)
