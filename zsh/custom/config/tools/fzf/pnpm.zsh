...
zpni() {
  ...
  while [[ $items ]] {
    items=$(npm search --searchlimit=200 --searchopts=sortBy=popularity react | awk -F "|" "{print \$1 \$2 \$5 \$6}" | fzf-tmux --header-lines=1 --nth=1,2,4 --ansi -m --preview "npm view --color {1}") || return
